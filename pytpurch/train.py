...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        data_iter = tqdm(data_train, mininterval=5)

        for n_iter, (imgs, labels) in enumerate(data_iter):
            logits = model(imgs)
            ...
            if n_iter % 50 == 0:
                data_iter.desc = train_desc.format(
                    epoch=epoch,
        ...
        data_iter = tqdm(data_val, mininterval=5)
        with torch.no_grad():
            for n_iter, (imgs, labels) in enumerate(data_iter):
                logits = model(imgs)
                ...
                if n_iter % 50 == 0:
                    data_iter.desc = val_desc.format(
                        epoch=epoch,
                        num_epochs=num_epochs,
                        n_iter=n_iter,
                        num_iters=len(data_train),
